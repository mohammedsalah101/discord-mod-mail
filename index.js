...
const watermark = 'Made by Dank Grinders- https://discord.gg/Qph6eG66pS
...
client.on("message", async (message) => {
    ...
    if (command === 'help') {
        ...
        const helpEmbed = new MessageEmbed()
            ...
            .setDescription('**Commands:**\n`_help` - This command that you are currently viewing.\n`_gd (text)` - Write text in geometry dash font.\n`_christmas` - Tells you how many days are until Christmas!\n`_banner (text)` - Write text on a p3nguin background.')
            .setFooter(`${watermark}`)
            .setColor(`${embedC}`)
            .setAuthor(`${embedA}`, `${embedP}`)
    ...
    if (findchannel && command != 'close' && message.channel.type === ('dm')) {
            ...
            guild.channels.create(`ticket-${message.author.username}`, { reason: 'New to Ticket' }).then(c => {
                ...
                if (Attachment[0] !== undefined) {
                    ...
                    const Imgembed = new MessageEmbed()
                        ...
                        .setTimestamp()
                        .setFooter('Type _close to close the ticket')
                        .setImage(pp1)
                        .setColor(`${embedC}`)
                        .setAuthor(`${embedA}`, `${embedP}`)
                    c.send(Imgembed);
        ...
        const NewTicket = new MessageEmbed()
            ...
            .setDescription('Your message has been sent to staff. To close your ticket, type `_close`')
